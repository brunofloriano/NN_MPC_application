function p_new = relaxing_pdf(p_old)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% begin user input %%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
function D = Initialize_vuw(D,G)
    for l=2:D.n
        ...
        v1 = -G.dif*r*cos(theta); % Potential Bowl
        D.v(l,1) = v1;
        D.u(l,1)=min(v1, 0);
        D.w(l,1)=max(v1, 0);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function D = my_initial_PDF(p,G)
    n = length(p); n2 = n^2;
    D.P = zeros(n2+1,1); D.P(1) = 0;
    
    D.key = zeros(n2+1,1); D.key(1) = -1;
    D.v = D.P; D.f = D.P; D.u = D.P; D.w = D.P; l = 2;
    x = G.x; y = G.y;
    for i = 1:n
        for j = 1:n
            state = [x(i);y(j)]/G.dx; D.key(l,1) = state_conversion(state, G);
            D.P(l,1) = p(i,j);
            l=l+1;
        end
    end
