...
MPCNNCar{1} = load('results\dataCar_Pi_1\dataCarnKMPC 2022-7-18-11-36.mat'); %Pi_1
MPCNNCar{2} = load('results\dataCar_Pi_2\dataCarnKMPC 2022-7-18-11-47.mat'); %Pi_2
MPCNNCar{3} = load('results\dataCar_Pi_3\dataCarnKMPC 2022-7-18-11-49.mat'); %Pi_3
MPCNNCar{4} = load('results\dataCar_Pi_4\dataCarnKMPC 2022-7-18-11-49.mat'); %Pi_4
...
dataCar = [];
for j = 1:4
    for i = 1:MPCNNCar{j}.N*MPCNNCar{j}.n    
        SMPCNNCar{j} = stepinfo(MPCNNCar{j}.state(i,:),MPCNNCar{j}.t);
        settling_timeMPCNNCar{j}(i) = SMPCNNCar{j}.SettlingTime;
        overshootMPCNNCar{j}(i) = SMPCNNCar{j}.Overshoot;
        mse_MPCNNCar{j}(i) = sum( (MPCNNCar{j}.state(i,:) - MPCNNCar{j}.state(i,end)).^2 )/length(MPCNNCar{j}.state(i,:));
    end
    dataCar = [dataCar [settling_timeMPCNNCar{j}'; mse_MPCNNCar{j}'] ];
end
